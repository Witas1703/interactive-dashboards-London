...
dashboardPage(
  ...
  dashboardBody(tabItems(
    tabItem(tabName = "start",
            mainPanel(
              fluidPage(
                ...
                h4("About"),
                p(
                  "The project itself is a part of our data visualization course at Poznan University of Technology. You can jump directly into universitiy's webpage by clicking the logo in the upper-right corner."
                ),
                br(),
                p(
                  "We decided to pick a data, that allow the user to see how COVID-19 hit London, one of the biggest cities in the world. Using our interactive widgets, one can see changes & current trends in vaccinations, cases, deaths and so on."
                ),
                br(),
                p(
                  "In addition, we decided to create a basic classification model, for a very rough COVID-19 symptoms checker. We used gini impurity measure to select splits when performing classification. If you're unfamiliar, please refer to ",
                  a(href = "https://www.gormanalysis.com/blog/magic-behind-constructing-a-decision-tree/", "this article.")
                ),
                hr(),
                h4("General information"),
                p(
                  "Most of our visualizations are interactive. Try clicking, mousovering and dive deeper into the data!"
                )
              )
            )),
    tabItem(
    ...
    tabItem(
      ...
      fluidPage(
        theme = shinytheme("yeti"),
        h3("Covid cases in England and London"),
        fluidRow(box(
          width = 12,
          uiOutput("slider"),
          plotlyOutput("pie")
        ...
        fluidRow(box(
          width = 12,
          p(em("click on pie chart to get a map!"))
        )),
        uiOutput("plot")
      )
    ),
      ...
      fluidPage(
        ...
        fluidRow(
          ...
          fluidRow(box(width = 6,
                       textOutput("percents1")),
                   box(width = 6,
                       textOutput("percents2")))
        )
      ),
      tabItem(
        tabName = "4",
        fluidPage(
          ...
          fluidRow(
            box(
              ...
              radioButtons(
                "soreThroat",
                label = "Do you experience sore throat?",
                choices = c("Yes" = "Yes", "No" = "No"),
                inline = TRUE
              ),
              radioButtons(
              ...
              radioButtons(
                "headAche",
                label = "Do you experience head ache?",
                choices = c("Yes" = "Yes", "No" = "No"),
                inline = TRUE
              )
            )
          ...
          fluidRow(box(
            ...
            p(
              em(
                "Please note, that above information is a mere prediction based on a computed model, in order to precisly determine your medical state, please contact medical services."
              )
            ),
            hr(),
            p(
              "Classification tree computed based on our data concerning COVID-19 symptoms, it allows us to rougly estimate, whether someone should test themselves."
            ),
            HTML(
              "<p>The tree should be read as following:</p> <br>
                       <ul>
          ...
          fluidRow(box(
            width = 12,
            h3("Should you make a COVID test?"),
